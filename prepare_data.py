...
def prepare_data(dataset_zip_dir, crossvalid_dir, numSubj, imageLen, windowLen, strideLen, n_moves, window_specs):
    ...
    if os.path.isfile(str(Path(crossvalid_dir,
                               'ICH_DataSegmentV1.pkl'))) == False:  # means the cross-validation folds was created before
        ...
        if os.path.exists(dataset_zip_dir) == True or os.path.exists(str(datasetDir)) == True:
            ...
            subject_nums_shaffled = [31, 39, 21, 33, 34, 5, 54, 2, 67, 68, 53, 29, 44, 76, 59, 73, 77, 71, 61, 69, 50,
                                     32, 6, 37,
            ...
            for cvI in range(0, NumCV):
                ...
                if NumCV != 1:
                    if cvI < NumCV - 1:
                        subjectNums_cvI_testing = subject_nums_shaffled[
                                                  cvI * int(numSubj / NumCV): cvI * int(numSubj / NumCV) + int(
                                                      numSubj / NumCV)]
                        subjectNums_cvI_trainVal = np.delete(subject_nums_shaffled, range(cvI * int(numSubj / NumCV),
                                                                                          cvI * int(
                                                                                              numSubj / NumCV) + int(
                                                                                              numSubj / NumCV)))
                    else:
                ...
                for subItrain in list(range(0, int(0.1 * total_trainval))) + list(
                    ...
                    NoHemorrhage = hemorrhage_diagnosis_array[
                        hemorrhage_diagnosis_array[:, 0] == subjectNums_cvI_trainVal[subItrain], 7]
                    ...
                    for sliceI in range(0, sliceNums.size):
                        if NoHemorrhage[
                            sliceI] == 0 or sliceI == randomSlice:  # Saving only the windows that have hemorrhage or
                            ...
                            for i in range(n_moves * n_moves):
                                if sum(sum(x_mask_segment[:, :, i])) > 0:
                                    imsave(
                                        Path(crossvalid_dir, 'CV' + str(cvI), 'train', 'image', str(counterI) + '.png'),
                                        np.uint8(x_ct_segmented[:, :, i]))
                                    imsave(
                                        Path(crossvalid_dir, 'CV' + str(cvI), 'train', 'label', str(counterI) + '.png'),
                                        np.uint8(x_mask_segment[:, :, i]))
                                    counterI = counterI + 1
                ...
                for subIvalidate in range(int(0.1 * total_trainval), int(0.3 * total_trainval)):
                    ...
                    NoHemorrhage = hemorrhage_diagnosis_array[
                        hemorrhage_diagnosis_array[:, 0] == subjectNums_cvI_trainVal[subIvalidate], 7]
                    SlicesWithoutHemm = np.nonzero(NoHemorrhage)
                    for sliceI in range(0, sliceNums.size):
                        if NoHemorrhage[sliceI] == 0:  # Saving only the windows that have hemorrhage
                            ...
                            for i in range(n_moves * n_moves):
                                if sum(sum(x_mask_segment[:, :, i])) > 0:
                                    imsave(Path(crossvalid_dir, 'CV' + str(cvI), 'validate', 'image',
                                                str(counterI) + '.png'), np.uint8(x_ct_segmented[:, :, i]))
                                    imsave(Path(crossvalid_dir, 'CV' + str(cvI), 'validate', 'label',
                                                str(counterI) + '.png'), np.uint8(x_mask_segment[:, :, i]))
                                    counterI = counterI + 1
                ...
                for subItest in range(0, len(subjectNums_cvI_testing)):
                    ...
                    for sliceI in range(0, sliceNums.size):
                        ...
                        imsave(Path(crossvalid_dir, 'CV' + str(cvI), 'test', 'fullCT', 'image',
                                    str(subjectNums_cvI_testing[subItest])
                                    + '_' + str(counterI) + '.png'), np.uint8(ct_scan[:, :, sliceI]))
                        imsave(Path(crossvalid_dir, 'CV' + str(cvI), 'test', 'fullCT', 'label',
                                    str(subjectNums_cvI_testing[subItest])
            ...
            with open(str(Path(crossvalid_dir, 'ICH_DataSegmentV1.pkl')),
                      'wb') as Dataset1:  # Python 3: open(..., 'wb')
                pickle.dump(
